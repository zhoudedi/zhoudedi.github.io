...
const helpContent = {
	shellType: {
		header: '烟花类型',
		body: '你要放的烟花的类型:“随机（Random）		裂纹（Crackle）克罗斯特（Crossette）菊花（Crysanthemum）落叶（Falling Leaves）花的（Floral）鬼（Ghost）马尾（Horse Tail）棕榈（Palm）戒指（Ring）频闪（Strobe）柳（Willow）、选择“随机（Random）”可以获得非常好的体验！'
	},
	shellSize: {
		header: '烟花大小',
